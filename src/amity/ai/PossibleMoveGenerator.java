...
{
	public static List<Movement> possibleBoards(Board board, Piece pieceAdd, Movement upper, int limitHeight)
	{
		...
		do
		{
			...
			for (int x = 0; x < xBound; x++)
			{
				...
				if ((y < yBound) && board.canPlace(current, x, y))
				{
					...
					Movement m = new Movement(x, testBoard, current, upper);
...
//						Movement mR = new Movement(x, curX, testBoard, current, upper);
//
//						// checks if allMovements already has a movement with a final board equal to this one
//						// skip if so b/c not necessary to have multiple ways to get there - waste of time
//						if (!allMovements.contains(new Movement(0, 0, testBoard, current, upper)))
//						{
//							allMovements.add(mR);
//						}
//					}
