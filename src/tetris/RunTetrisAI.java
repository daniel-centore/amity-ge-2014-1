...
public class RunTetrisAI extends RunTetris {
    ...
    public boolean tickAI() {
        if (this.current_count != this.tc.count) {
            ...
            this.mMove = this.mBrain.bestMove(new Board(this.tc.board),
                    this.tc.currentMove.piece, this.tc.nextPiece,
                    this.tc.board.getHeight() - TetrisController.TOP_SPACE);
        }
        ...
        if (!this.tc.currentMove.piece.equals(this.mMove.piece)) {
            ...
            super.tick(this.tc.currentMove.x < this.mMove.x ? TetrisController.RIGHT
